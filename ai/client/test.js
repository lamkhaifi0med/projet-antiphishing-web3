...
const {
  validateOutput,
  hasUnnegatedStrongSignal,
} = require("./lib/validateOutput");
...
async function testGeminiFallbackOn429() {
  ...
  process.env.NVIDIA_MODEL_FALLBACK =
    process.env.NVIDIA_MODEL_FALLBACK || "meta/llama-3.1-8b-instruct";
  ...
  const validOutput = {
    ...
    explanation:
      "Réponse NVIDIA simulée pour tester la bascule automatique sur une erreur 429 de Gemini (RF-A6, critère d'acceptation n°3).",
  };
  ...
  globalThis.fetch = async (url) => {
    ...
    if (target.includes("generativelanguage.googleapis.com")) {
      ...
      return {
        ok: false,
        status: 429,
        json: async () => ({ error: { message: "quota exceeded (mock)" } }),
      };
    }
    if (
      target.includes("integrate.api.nvidia.com") ||
      target.includes(process.env.NVIDIA_BASE_URL)
    ) {
      nvidiaCalls += 1;
      return {
        ok: true,
  ...
  try {
    ...
    assert.equal(
      geminiCalls,
      1,
      "Gemini doit être appelé une fois (et échouer en 429)",
    );
    assert.equal(
      nvidiaCalls,
      1,
      "NVIDIA doit être appelé une fois, suite à la bascule",
    );
    assert.equal(
      result.modelUsed,
      "nvidia",
      "(c) le modèle réellement utilisé doit être journalisé comme 'nvidia'",
    );
    ...
    assert.ok(
      valid,
      `(b) la sortie doit rester conforme au schéma : ${errors.join(", ")}`,
    );
...
async function testShortContentStillCallsLlm() {
  ...
  const urlOnlyOutput = {
    ...
    explanation:
      "Contenu de page indisponible ; verdict fonde uniquement sur les features URL.",
  };
  ...
  globalThis.fetch = async (url) => {
    ...
    if (target.includes("generativelanguage.googleapis.com")) {
      ...
      return {
        ...
        json: async () => ({
          candidates: [
            { content: { parts: [{ text: JSON.stringify(urlOnlyOutput) }] } },
          ],
        }),
      };
    }
  ...
  try {
    ...
    assert.equal(
      geminiCalled,
      true,
      "le LLM doit etre appele meme sans contenu de page exploitable (mode url_only)",
    );
    ...
    assert.notEqual(
      result.modelUsed,
      null,
      "un contenu court ne doit plus produire modelUsed=null (ancien court-circuit force)",
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
async function testSuspiciousTriggersManualReview() {
  ...
  const suspiciousOutput = {
    ...
    explanation:
      "Ton pressant sans preuve technique suffisante pour conclure a malicious.",
  };
    ...
    if (target.includes("generativelanguage.googleapis.com")) {
      return {
        ...
        json: async () => ({
          candidates: [
            {
              content: { parts: [{ text: JSON.stringify(suspiciousOutput) }] },
            },
          ],
        }),
      };
  ...
  try {
    ...
    assert.equal(
      result.manualReview,
      true,
      "un verdict suspicious valide doit porter manualReview=true",
    );
  } finally {
    globalThis.fetch = originalFetch;
  }
...
async function testStructuralEvidencePromotesMode() {
  ...
  const output = {
    ...
    confidence: 0.85,
    category: "wallet_drainer",
    indicators: ["Motif eth_sign detecte dans un script"],
    explanation:
      "Digest structurel revele une demande de signature suspecte malgre un texte visible court.",
  };
  ...
  try {
    const result = await analyze({
      ...
      structuralDigest: {
        formFields: [],
        externalScriptDomains: [],
        web3PatternSnippets: [{ pattern: "eth_sign", context: "..." }],
      },
    });
...
function testValidateOutputRequiresCategoryKey() {
  ...
  assert.equal(
    valid,
    false,
    "category doit etre une propriete requise meme absente pour un verdict non-malicious",
  );
  assert.ok(
    errors.some((e) => e.includes("category")),
    `attendu une erreur mentionnant category, recu : ${errors.join(", ")}`,
  );
}
...
function testStrongSignalNegationHandling() {
  assert.equal(
    hasUnnegatedStrongSignal(
      "No typosquatting detected, only urgency language.",
    ),
    false,
    "une negation explicite ne doit pas declencher le signal fort",
  );
  assert.equal(
    hasUnnegatedStrongSignal("Aucun typosquatting observe sur ce domaine."),
    false,
    "negation en francais non plus",
  );
  assert.equal(
    hasUnnegatedStrongSignal(
      "Typosquatting confirme : blnance.com imite binance.com.",
    ),
    true,
    "une preuve forte reellement affirmee doit toujours declencher le signal",
  );
  ...
  assert.ok(
    valid,
    `une explication qui NIE une preuve forte ne doit pas etre rejetee : ${errors.join(", ")}`,
  );
}

/**
 * Fiabilite du mode force (evaluate.js --provider=gemini) : un 429
 * transitoire avec delai conseille (retryDelay Gemini) doit etre reessaye
 * apres ce delai, sans bascule NVIDIA ni exclusion provider_error.
 */
async function testForcedGeminiRetriesTransient429() {
  process.env.GEMINI_API_KEY = "test-dummy-gemini-key";
  ...
  const output = {
    ...
    indicators: [],
    explanation:
      "Reponse simulee apres un 429 transitoire pour tester le budget de retries dedie aux erreurs fournisseur.",
  };
    ...
    if (target.includes("generativelanguage.googleapis.com")) {
      ...
      if (geminiCalls === 1) {
        return {
          ...
          json: async () => ({
            error: {
              message: "quota exceeded (mock)",
              details: [{ retryDelay: "0.05s" }],
            },
          }),
        };
      }
  ...
  try {
    const result = await analyze(
      {
        url: "https://mock-transient-retry-test.invalid/",
        textExcerpt: "x".repeat(250),
        structuralDigest: {
          formFields: [],
          externalScriptDomains: [],
    ...
    assert.equal(geminiCalls, 2, "Gemini doit etre reessaye apres le 429");
    assert.equal(nvidiaCalls, 0, "aucune bascule NVIDIA en mode force");
...
/**
 * Epuisement du budget transitoire : des 429 persistants doivent finir en
 * forcedSuspicious (manualReview, modelUsed=null), jamais en crash.
 */
async function testForcedGeminiTransientBudgetExhaustion() {
  process.env.GEMINI_API_KEY = "test-dummy-gemini-key";
  ...
  globalThis.fetch = async (url) => {
    ...
    if (target.includes("generativelanguage.googleapis.com")) {
      return {
        ...
        json: async () => ({
          error: {
            message: "quota exceeded (mock, persistant)",
  ...
  try {
    const result = await analyze(
      {
        url: "https://mock-transient-exhaustion-test.invalid/",
        textExcerpt: "x".repeat(250),
        structuralDigest: {
          formFields: [],
          externalScriptDomains: [],
...
async function run() {
  console.log(
    "Critère d'acceptation n°3 — fallback NVIDIA sur 429 Gemini (mock, sans réseau réel)",
  );
  await test(
  ...
  await test(
    "AI recall v2 : contenu court -> mode url_only, LLM quand meme appele",
    testShortContentStillCallsLlm,
  );
  await test(
    "AI recall v2 : preuve structurelle -> mode url_structural",
    testStructuralEvidencePromotesMode,
  );
  await test(
    "RF-N9 : verdict suspicious valide -> manualReview=true",
    testSuspiciousTriggersManualReview,
  );
  await test("RF-A4/A5 : category est une propriete requise (meme absente, pas seulement invalide)", async () =>
    testValidateOutputRequiresCategoryKey());
  await test("RF-A4 §8.3 : negation d'une preuve forte n'est pas la preuve elle-meme", async () =>
    testStrongSignalNegationHandling());
  await test(
    "Fiabilite : 429 transitoire en mode force -> retry apres retryDelay, sans provider_error",
    testForcedGeminiRetriesTransient429,
  );
  await test(
    "Fiabilite : 429 persistants -> forcedSuspicious apres epuisement du budget transitoire",
    testForcedGeminiTransientBudgetExhaustion,
  );
