...
async function main() {
  ...
  try {
    ...
    response = await request(baseUrl, "/check", {
      type: "email",
      value: "a@b.test",
    });
    ...
    assert.equal(
      calls.length,
      0,
      "une entree invalide ne doit jamais appeler check.js",
    );
    ...
    response = await request(baseUrl, "/check", {
      type: "url",
      value: "https://listed.invalid/path",
    });
    assert.equal(response.status, 200);
    ...
    const lines = fs
      .readFileSync(path.join(tempDir, "reports.jsonl"), "utf8")
      .trim()
      .split(/\r?\n/);
    assert.equal(
      lines.length,
      1,
      "un reportId ne doit etre journalise qu'une fois",
    );
    ...
    const concurrentReport = {
      ...report,
      reportId: "r_20260809T120001Z_concurrent",
      createdAt: "2026-08-09T12:00:01.000Z",
    };
    const concurrentResponses = await Promise.all([
      request(baseUrl, "/reports", concurrentReport),
      request(baseUrl, "/reports", concurrentReport),
    ]);
    ...
    assert.deepEqual(
      concurrentResponses.map((candidate) => candidate.status).sort(),
      [200, 201],
      "une seule creation concurrente doit retourner 201",
    );
    assert.deepEqual(
      concurrentBodies.map((body) => body.created).sort(),
      [false, true],
      "created doit refleter le resultat atomique de chaque requete",
    );

    const concurrentLines = fs
      .readFileSync(path.join(tempDir, "reports.jsonl"), "utf8")
      .trim()
      .split(/\r?\n/);
    assert.equal(
      concurrentLines.filter(
        (line) => JSON.parse(line).reportId === concurrentReport.reportId,
      ).length,
      1,
      "un doublon concurrent ne doit etre persiste qu'une fois",
    );
    ...
    assert.equal(
      response.status,
      400,
      "un statut hors RF-N10 doit etre refuse",
    );
    ...
    assert.equal(
      updatedLines.length,
      2,
      "les transitions ne doivent pas dupliquer la ligne du rapport",
    );

    console.log("WF1/WF2 bridge: controles sequentiels et concurrents passes.");
  } finally {
