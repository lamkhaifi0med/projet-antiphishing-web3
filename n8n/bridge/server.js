...
const { HttpError, validateCheckRequest, validateReportRecord, validateReportUpdate } = require("./lib/validation");
...
function requireAuthorization(request, secret) {
  ...
  if (!header.startsWith("Bearer ") || !tokensEqual(header.slice(7), secret)) {
    throw new HttpError(401, "unauthorized", "Authentification bridge invalide.");
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("data", (chunk) => {
      ...
      if (size > MAX_BODY_BYTES) {
        reject(new HttpError(413, "body_too_large", "Corps JSON trop volumineux."));
        request.destroy();
        return;
      }
      chunks.push(chunk);
...
function createBridgeServer({ secret, check = runCheck, journal }) {
  if (typeof secret !== "string" || secret.length < 32) {
    throw new Error("BRIDGE_SHARED_SECRET doit contenir au moins 32 caracteres.");
  }
  ...
  return http.createServer(async (request, response) => {
    try {
      ...
      if (reportRoute && request.method === "GET") {
        ...
        if (!record) throw new HttpError(404, "report_not_found", "Signalement introuvable.");
      ...
      console.error(JSON.stringify({ event: "bridge_error", message: error.message }));
      sendJson(response, 502, { error: "bridge_failure", message: "Lecture blockchain indisponible." });
    }
  });
}
...
function main() {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error("BRIDGE_PORT invalide.");

  const journalPath = process.env.REPORT_JOURNAL_PATH || path.resolve(__dirname, "data/reports.jsonl");
  const server = createBridgeServer({ secret, journal: createJournal(journalPath) });
...
module.exports = { MAX_BODY_BYTES, createBridgeServer, readJsonBody, tokensEqual };
